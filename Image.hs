...
fillImageLists :: (Storable a, NDims s) => Int -> Int -> [[a]] -> Image s a
...
savePNG :: (NDims s, ND s ~ Two) => FilePath -> Image s (Int,Int,Int) -> IO ()
savePNG fp img = do
...
c _ = (100,100,100)
data Flatland
instance NDims Flatland where 
    type ND Flatland = Two
